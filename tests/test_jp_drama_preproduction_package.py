...
def _import_series(tmp_path: Path) -> tuple[Path, Path, Path]:
    ...
    assert import_main(
        [
            "--series-plan", str(series_plan),
            "--asset-catalog", str(asset_catalog),
            "--output-dir", str(output),
            "--live-provider-config", str(LIVE_CONFIG),
            "--minimax-h3-config", str(H3_CONFIG),
...
def _build_package(tmp_path: Path) -> Path:
    ...
    assert package_main(
        [
            "--series-output", str(series_output),
            "--source-series-plan", str(series_plan),
            "--source-asset-catalog", str(asset_catalog),
            "--live-provider-config", str(LIVE_CONFIG),
            "--output-dir", str(output),
        ]
    ) == 0
...
def test_real_one_bunch_preproduction_package_is_complete_and_zero_call(
    ...
    output = _build_package(tmp_path)
    ...
    assert (
        ...
        manifest.base_master_asset_count,
        manifest.voice_identity_count,
        manifest.first_frame_count,
        manifest.provider_route_count,
    ) == (
        "一房の葡萄",
        SOURCE_COMMIT,
        3,
    ...
    assert manifest.contract_ready and manifest.provider_plans_ready
    assert not manifest.master_assets_ready
    assert not manifest.voices_ready
    assert not manifest.first_frames_ready
    assert not manifest.video_generation_ready
    ...
    assets = _json(output, "asset_creation_checklist.json")
    by_id = {item["source_asset_id"]: item for item in assets}
    assert len(assets) == 17 and set(by_id) == ASSET_IDS
    ...
    voices = _json(output, "voice_identity_checklist.json")
    assert {item["source_character_id"] for item in voices} == {
        "C01", "C02", "C03", "C91"
    }
    assert all(item["status"] == "pending" for item in voices)

    frames = _json(output, "first_frame_plan.json")
    assert [item["segment_id"] for item in frames] == [
        f"E{episode:02d}-G{segment:02d}"
        for episode in range(1, 4)
        for segment in range(1, 6)
    ]
    assert all(1 <= len(item["master_reference_asset_ids"]) <= 9 for item in frames)
    assert all("--stage preflight" in item["keyframe_preflight_command"] for item in frames)
    assert all("--execute-paid" in item["keyframe_paid_command_template"] for item in frames)
    assert all(
        "<COPY_APPROVAL_DIGEST_FROM_PREFLIGHT>" in item["keyframe_paid_command_template"]
        for item in frames
    )

    routes = _json(output, "provider_route_summary.json")
    assert len(routes) == 9
    ...
    assert all(item["planning_ready"] and item["execution_route_ready"] for item in routes)
    assert all(
        item["cost"]["unknown_cost_components"]
        for item in routes
        if item["route"] == "seedance"
    )

    canary = _json(output, "canary_recommendation.json")
    assert canary["recommendation_ready"] is False
    assert canary.get("recommended_episode_id") is None
    assert canary.get("recommended_segment_id") is None
    ...
    commands = _json(output, "bundle_approval_commands.json")
    assert len(commands) == 9
    assert all("approve_asset_bundle" in item["command"] for item in commands)
...
def test_preproduction_build_failure_preserves_previous_output(tmp_path: Path) -> None:
    ...
    assert package_main(
        [
            "--series-output", str(series_output),
            "--source-series-plan", str(series_plan),
            "--source-asset-catalog", str(broken_catalog),
            "--live-provider-config", str(LIVE_CONFIG),
            "--output-dir", str(output),
            "--overwrite",
        ]
