...
def _write_video(
    ...
    *,
    ...
    arguments = [
        ...
        "lavfi",
        "-i",
        f"testsrc2=size=90x160:rate={fps}:duration={seconds}",
    ]
    if with_audio:
        arguments.extend(
            [
...
def test_h3_approved_assets_publish_reuse_and_materialize(tmp_path: Path) -> None:
    ...
    assert first.external_storage_uploads == len(preflight.items)
    ...
    assert len(publisher.uploads) == len(preflight.items)
    assert len(publisher.signatures) == len(preflight.items) * 2


def test_h3_publication_rejects_wrong_digest_and_expiring_urls(tmp_path: Path) -> None:
    prepared, plan = _route("h3")
    bundle = _approve_h3_masters(prepared, plan, tmp_path)
    preflight = build_h3_asset_publication_preflight(
        prepared,
...
@FFMPEG_REQUIRED
def test_manual_seedance_output_is_approval_bound_and_revalidated(
    tmp_path: Path,
) -> None:
...
@FFMPEG_REQUIRED
def test_wan_and_h3_evidence_bind_ledgers_and_final_mp4(tmp_path: Path) -> None:
    for route, evidence_kind, with_audio in (
        ...
        approval_path = tmp_path / route / "approval.json"
        report = {
            "valid": True,
            "stage": "render",
        ...
        if route == "wan":
            ...
            request_approval = {"shot_id": segment.segment_id}
        else:
            ledger = {
                "segment_id": segment.segment_id,
            ...
            request_approval = {"segment_id": segment.segment_id}
        report_path.write_text(json.dumps(report), encoding="utf-8")
        ledger_path.write_text(json.dumps(ledger), encoding="utf-8")
        approval_path.write_text(json.dumps(request_approval), encoding="utf-8")
        evidence = SegmentEvidence(
            kind=evidence_kind,
...
def test_artifact_manifest_rejects_wrong_order() -> None:
    _, plan = _route("wan")
...
def test_segment_import_cli_requires_exact_preflight_digest(tmp_path: Path) -> None:
    ...
    artifact = SegmentArtifact.model_validate_json(
        (result_dir / f"{segment.segment_id}.segment_artifact.json").read_text(
            encoding="utf-8"
        )
    )
